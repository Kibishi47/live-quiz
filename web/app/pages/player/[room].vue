<template>
  ...
  <!-- CHAT DRAWER TOGGLE BUTTON & DRAWER OUTSIDE CONTAINER -->
  <template v-if="isConnected && gameState && nickname">
    <button 
    ...
    <div class="chat-drawer" :class="{ 'open': showChatDrawer }">
      ...
      <div class="chat-messages" ref="chatScrollContainer">
        <div v-if="!gameState.chat || gameState.chat.length === 0" class="no-messages">
          Aucun message. Envoyez le premier message !
        </div>
        <div 
          v-else 
          v-for="(msg, idx) in gameState.chat" 
          :key="idx" 
          class="chat-bubble-wrapper"
          :class="{ 'is-me': isMe(msg.sender), 'is-host': msg.sender === 'Hôte' }"
        >
...
// Watch chat messages from gameState
watch(
  () => gameState.value?.chat,
  (newChat, oldChat) => {
    if (!newChat) return
    if (!showChatDrawer.value) {
